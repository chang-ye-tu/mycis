...
splash = QSplashScreen(QPixmap('res/img/health_care_shield.png'))
splash.show()
app.processEvents()
...
MED_CODE_FAV, MED_NAME_FAV, MED_MANUFACTURER, MED_DOSAGE_FAV, MED_FREQ_FAV, MED_DAYS_FAV, MED_USAGE_FAV, MED_OCODE_FAV = range(8)
...
PRICE_DATE, PRICE_PRICE = range(2)
MED_ID_SELF, MED_CODE_SELF, MED_NAME_SELF = range(3) 
ORD_ID_SELF, ORD_CODE_SELF, ORD_NAME_SELF = range(3) 

CLINIC_NAME = u'杜外科診所'                     #
CLINIC_CODE = u'3521011039'                     #
CLINIC_TAX_NR = u'69564144'                     # 
CLINIC_ZIP = u'70149'                           #
CLINIC_ADDRESS = u'台南市東區東門路二段六十三號'#
CLINIC_TEL = '06-2378927'                       #
DEPT_CODE= '03'                                 #
DEPT_NAME = u'外科'                             #
DOCTOR_PID = u'E101919343'                      #
DOCTOR_NAME = u'杜進旺'                         #
DOCTOR_CODE = u'037964'                         #
PHARMACIST_PID = u'D121350345'                  #
PHARMACIST_NAME = u''                           #
OPREP_CODE = '1'                                #
...
from ui.dlg_self import Ui_dlg_self
from ui.dlg_price import Ui_dlg_price
...
from ui.wdg_self import Ui_wdg_self

print('load ui files: %f seconds.' % (time.time() - start_time0,))

show_msg(u'載入 icd10 快取檔...')
start_time0 = time.time()
...
ix_whoosh = open_dir(os.path.join(os.getcwd(), 'cache', 'dir_indices'))
print('load whoosh def: %f seconds.' % (time.time() - start_time0,))

def update_dic_custom_code_name(cr, staff_id=1):
    TK = ['diag', 'ord', 'med', 'mat']
    dd = {}
    r = cr.execute('select id, code, name from fav where staff_id = ?', (staff_id,)).fetchall()
    for fav_id, fav_code, fav_name in r:
        d = {}
        for t in ['diag', 'ord', 'med', 'mat']:
            d[t] = cr.execute('select %s_id from fav_%s where fav_id = ?' % (t, t), (fav_id,)).fetchall()
...
show_msg(u'載入常用存取字典檔 ...')
start_time0 = time.time()
dic_all = defaultdict(dict)
dic_all_inv = defaultdict(dict)
for t in ('sam', 'amend', 'tktype', 'insttype', 'selfpay', 'ocode', 'state', 
          'usage', 'freq', 'diag', 'ord', 'med', 'mat', 'anormaly'):
    for r in cr.execute('select * from %s' % t).fetchall():
        dic_all[t][r[1]] = r[0]
        dic_all_inv[t][r[0]] = r[1:]
...
def validate_effect(ded, par):
    date = dt_tw(ded.date().toPyDate(), date_only=True)
    try:
        w = par.centralWidget()
        i = w.currentIndex()
        l = w.widget(i).tbv.selectedIndexes()
        d = par.d_selfs[i][l[0].row()]['price']
        dates = [dd[0] for dd in d]
        if date in dates:
            raise error, u'此日期已在目前生效日期表中！'
        elif date < max(dates):
            raise error, u'請輸入更大生效日期！'
        return 1 

    except error, e:
        QMessageBox.warning(ded.parent(), u'輸入生效日期錯誤', unicode(e))
        ded.setFocus()
        return 0
...
def serial_s(serial, insttype_code, sign, amend_code, tktype_code):
    ...
    if s:
        if insttype_code == 'B6':
            ...
            h = u'補卡：' 
        elif s in dic_all_inv['anormaly']:
            h = u'異常：'
...
def age(s):
    born = str2date(str(int(s[:3]) + 1911) + s[3:])
...
def ids(l, n=2):
    return [tuple(ll[:n]) for ll in l]

def ids_(l, key='fav_id'):
    return [ll[key] for ll in l]

def list_in_ids(l, _ids, n=2):
    return [tuple(ll) for ll in l if tuple(ll[:n]) in _ids]

def list_in_ids_(l, _ids, key='fav_id'):
    return [ll for ll in l if ll[key] in _ids]

def diff(l_i, l_f, n=2, b_debug=False):
    if b_debug:
        print('l_i: ', l_i)
        print('l_f: ', l_f)
    ...
    l_i_in = list_in_ids(l_i, k_i & k_f, n) 
    l_f_in = list_in_ids(l_f, k_i & k_f, n)

    assert sorted(ids(l_i_in, n)) == sorted(ids(l_f_in, n))

    l_delete = list_in_ids(l_i, k_i - k_f, n)
    l_insert = list_in_ids(l_f, k_f - k_i, n)
...
def diff_favs(favs_i, favs_f):
    ...
    d = {'l_delete': l_delete, 'l_insert': l_insert, 'l_update_i': l_i_in, 'l_update_f': l_f_in}
    return d

def diff_d_selfs(d_self_i, d_self_f):
    k_i = set(ids_(d_self_i, key='id'))
    k_f = set(ids_(d_self_f, key='id'))

    l_i_in = list_in_ids_(d_self_i, k_i & k_f, key='id') 
    l_f_in = list_in_ids_(d_self_f, k_i & k_f, key='id')

    assert sorted(ids_(l_i_in, key='id')) == sorted(ids_(l_f_in, key='id'))

    l_delete = list_in_ids_(d_self_i, k_i - k_f, key='id')
    l_insert = list_in_ids_(d_self_f, k_f - k_i, key='id')
...
def import_tk(tk_id, cr):
    ...
    r = cr.execute('''select code from tel 
                        join patient_tel on patient_tel.tel_id = tel.id
                        where patient_tel.patient_id = 
                        (select patient_id from tk where tk.id = ?)''', (tk_id,)).fetchall()
    d['tel'] = r[0][0] if r else ''

    r = cr.execute('''select code from addr 
                    join patient_addr on patient_addr.addr_id = addr.id
                    where patient_addr.patient_id = 
                    (select patient_id from tk where tk.id = ?)''', (tk_id,)).fetchall()
    d['addr'] = r[0][0] if r else ''

    for t in ['diag', 'ord', 'med', 'mat']:
        d[t] = []
        for i in cr.execute('select * from tk_%s where tk_id = ?' % t, (tk_id,)).fetchall():
            d[t].append(list(i))
    return d
...
def import_selfs(cr):
    d = {}
    for ii, t in enumerate(['med', 'ord']):
        d[ii] = []
        for i in cr.execute('''select id, code, name from %s 
                               where code like "%s***%%" 
                               order by id''' % tuple([t]*2)).fetchall():
            dd = {} 
            r = cr.execute('''select effect.effective_date, %s_effect.price 
                              from %s_effect 
                              join effect on %s_effect.effect_id = effect.id
                              where %s_effect.%s_id = ? 
                              order by effect.effective_date''' % tuple([t]*5), 
                              (i[0],)).fetchall()
            dd['id'], dd['code'], dd['name'] = i
            dd['price'] = [list(rr) for rr in r]
            d[ii].append(dd)
    return d
...
def set_typ(code):
    typ = 'none'
    for t in ['ord', 'med', 'mat',]: # mat are few 
        if code in dic_all[t]:
            typ = t
            break
    return typ
...
def tk_save(name='', pid='', birthday='', card_code='', insurertype_code='', state_code='3', newborn='', newbornmark='', newbornprobe='', tktype_code='01', amend_code='1', dt='', serial='', sign='', sam_code='', insttype_code='09', staff_id_doctor=1, staff_id_pharmacist=2, inst_id=0):    
    ...
    try:
        ...
        if r:
            ...
            cr.execute('insert into patient (pid, name, birthday) values (?, ?, ?)', (pid, name, birthday))
            patient_id = cr.execute('select last_insert_rowid()').fetchone()[0]

        if not inst_id:
            cr.execute('''insert into inst (insttype_id) values (?)''', (insttype_id,)) 
            
            inst_id = cr.execute('select last_insert_rowid()').fetchone()[0] 
        
        r = cr.execute('select id from card where code = ?', (card_code,)).fetchone()
        if r:
            card_id = r[0]
        else:
...
def attrs(cr, tk_id=0, inst_id=0, ym='', nr=0, is_app=False, is_xml=False, is_ic=False):
    ...
    # XXX   
    r = cr.execute('''select patient.id,
                             patient.name, 
                             patient.pid, 
                             patient.birthday,
                             inst.is_cont, 
    ...
    patient_id, name, pid, birthday, is_cont, is_severe, insttype_code = r 

    r = cr.execute('''select code from tel 
                    join patient_tel on patient_tel.tel_id = tel.id
                    where patient_tel.patient_id = ?''', (patient_id,)).fetchall()
    tel = r[0][0] if r else ''

    r = cr.execute('''select code from addr 
                    join patient_addr on patient_addr.addr_id = addr.id
                    where patient_addr.patient_id = ?''', (patient_id,)).fetchall()
    addr = r[0][0] if r else ''
    ...
    for g in r_tk:
        ...
        if s: # 卡序掛號：計價 
            ...
            if selfpay_code in ('003', '004', '005', '006', '901', '902', '903', '904'):
...
class dll(thread):
    ...
    def get_seq(self, amend_code='1', tktype_code='01', d=None):
        ...
        r = cs.hisGetSeqNumber256(c_char_p(fl(tktype_code, 3)), c_char_p(fl(d.get('newbornmark', ''), 2)), 
                                  c_char_p(fl(amend_code, 1)), buf, byref(c_int(sizeof(buf))))
        if r == 0:
    ...
    def write(self, tk_id=0, amend_code='1'):
        ...
        r = cs.hisWriteTreatmentData(c_char_p(fl(d['dt'], 14, True)), 
                                     ...
                                     c_char_p(''.join([fl(amend_code, 1), fl(d['icds_ic'], 30),
                                                       fl(d['amount_all'], 8), fl(d['amount_selfpay'], 8),
                                                       fl(0, 8), fl(0, 7), fl(0, 7),])),
        ...
        if r == 0:
            ...
            if 0 < n <= 60:
                ...
                r = cs.hisWriteMultiPrescriptSign(c_char_p(fl(d['dt'], 14, True)), 
                                                  ...
                                                  byref(c_int(n)), buf, byref(c_int(40 * n)))
                if r == 0:
                    s = mid(buf, 1, 40 * n)
...
TTFSearchPath.append(cat(os.getcwd(), 'res', 'fonts'))
...
def print_proxy(arg):   
    ...
    def chk(d, cv):
        ...
        s8 = [
            ...
            (6, 83, spaced_s(CLINIC_CODE)), 
            (6, 87, spaced_s(CLINIC_ZIP)),
            (6, 91, spaced_s(CLINIC_TEL)), 
        ]
        for s in s8:
            text(s, cv, size=8, font='msjh')

        s_i = [(32, 83, spaced_s(CLINIC_NAME, ' '*2)), (20, 87, spaced_s(CLINIC_ADDRESS)),]
        for s in s_i:
            text(s, cv)
        ...
        for s in s_n:
            text(s, cv, font='msjh', size=8)
        
        s14 = [(105, 14, spaced_s(CLINIC_NAME, ' '*2))]
        for s in s14:
            text(s, cv, size=14, centered=True)
    ...
    def rcpt_form(cv, scale=(1, 1), translation=(0, 0)):
        ...
        s9 = [
            ...
            (72,  79, u'部分負擔費用總計：'),
            (139, 79, u'療程實收費用總計：'),
            (116, 91, u'列  印  日  期  ：  中  華  民  國      年     月     日'),
            (6, 91, u'統  一  編  號')
        ]
        for s in s9:
            text(s, cv)
    ...
    def rcpt(d, cv):
        ...
        s8 = [
            ...
            (173, 91, dt_now().year - 1911),         # 列印年
            (185, 91, str(dt_now().month).zfill(2)), # 列印月
            (196, 91, str(dt_now().day).zfill(2)),   # 列印日
            (6, 83, spaced_s(CLINIC_CODE)), 
            (180, 83, spaced_s(CLINIC_TEL)), 
            (6, 87, spaced_s(CLINIC_ZIP)),
            (32, 91, spaced_s(CLINIC_TAX_NR)),
        ]
        for s in s8:
            text(s, cv, size=8, font='msjh')
    ...
    def rcptn_form(cv, scale=(1, 1), translation=(0, 0)):
        ...
        text((105, 13, spaced_s(u'%s門診醫療費用收據' % CLINIC_NAME)), cv, size=14, centered=True)
    ...
    def rcptn(d, cv):
        ...
        s_n = [(6, 83, spaced_s(CLINIC_CODE)), (6, 91, spaced_s(CLINIC_TEL)), (6, 87, spaced_s(CLINIC_ZIP)),]   
    ...
    def proof(d, cv):
        ...
        s = [
            ...
            (10,  254,   u'診治醫師'),  
            (10,  260,   u'醫師證書第        號'),
            (44,  248,   ' '.join(DOCTOR_NAME)),
            (44,  254,   ' '.join(DOCTOR_NAME)),
            (44,  274,   CLINIC_NAME),
            (10,  280,   CLINIC_ADDRESS),
            (10,  286,   u'中華民國       年      月       日'),
        ]
        for ss in s:
            text(ss, cv, size=16)
        ...
        text((105, 20, ' '.join(CLINIC_NAME)), cv, size=36, centered=True)
        text((105, 35, u'診   斷   證   明   書'), cv, size=36, centered=True)
        
        s = [
            (21,   50,   d['name']),                 # 姓名
            (123,  50,   gender(d['pid'], False)),   # 性別
            (21,   66,   dt_s(d['birthday'], True)), # 生日 
            (125,  66,   d['pid']),                  # 身份證號
            (21,   82,   d['addr']),                 # 住址 
            (21,   104,  d['dt_pr']),                # 應診日期 
            (118,  104,  DEPT_NAME),                 # 應診科別
        ]
        for ss in s:
    ...
    def referral(d, cv):
        ...
        s = [
            (21,   34.5,  d['name']),                # 姓名
            (65,   34.5,  gender(d['pid'], False)),  # 性別
            (112,  34.5,  dt_s(d['birthday'], True)),# 出生日期 
            (157,  34.5,  d['pid']),                 # 身份證號
            (21,   52.5,  d['name']),                # 聯絡人
            (65,   52.5,  d['tel']),                 # 聯絡電話
            (112,  52.5,  d['addr']),                # 聯絡住址 
            (21,   142,   DOCTOR_NAME),              # 診治醫師 
            (21,   150,   DEPT_NAME),                # 診治醫師（科別） 

            (21,   162,  CLINIC_CODE),  
            (21,   168,  CLINIC_NAME), 
            (117,  162,  CLINIC_ADDRESS),
            (117,  168,  CLINIC_TEL),  

            (21,   184,  dt_s(now(), True)),         # 轉診日期
            (117,  184,  dt_s(now(), True)),         # 開單日期
        ]
        for ss in s:
            text(ss, cv, size=12, font='msjh')
    
        text((43, 18, CLINIC_NAME), cv, size=18)
        text((8, 18, CLINIC_CODE), cv, size=16, font='msjh')
...
def printf(typ, tk_id, par):
    ...
    if typ in ('rcptn', 'dtl'):
        if qna(par, u'選擇%s列印模式' % zh(typ), u'%s列印：列印整個案件，或是僅列印此次掛號內容？\n\n選擇 [Yes] 列印整個案件，[No] 僅列印此次掛號內容！' % zh(typ)) == QMessageBox.Yes:
            r = cr.execute('select inst_id from tk where id = ?', (tk_id,)).fetchone()
            if r:
                inst_id = r[0]                
            ...
            for ct in INSTTYPE:
                ...
                r = cr.execute('''select inst.id, inst.insttype_id 
                   ...
                   and (exists (select 1 from tk_ord where tk_ord.tk_id = tk.id) or 
                        exists (select 1 from tk_med where tk_med.tk_id = tk.id)))
                                  and insttype.code = ?''', (self.ym + '%', ct)).fetchall()
                n_typ = len(r)
...
<outpatient><tdata>%s</tdata>%s</outpatient>''' % (tdata, ''.join(l_ddata))
            ...
            r = eii.NHI_SendB(c_int(com_port), c_char_p(reader_dll), c_char_p(zip_), c_char_p('05'), buf_lid, buf_nid)
            if r != 0:
            ...
            log(u'儲存申報資料中，請稍候....') 
            cr.execute('insert into app (ym, serial) values (?, ?)', (self.ym, now()))
            ...
            qry = cat(td, 'qry')
            open(qry, 'w').write('\n'.join(['FEE_YM=%s' % self.ym, 'APPL_TYPE=%s' % appl_type, 
                                            'HOSP_DATA_TYPE=%s' % hosp_data_type, 'APPL_DATE=']))
            
            buf_lid = create_string_buffer(12)
            buf_nid = create_string_buffer(12)
            r = eii.NHI_DownloadB(c_int(com_port), c_char_p(reader_dll), c_char_p(qry), c_char_p('03'), buf_lid, buf_nid)
            if r != 0:
...
class uploader(thread):
    ...
    def process(self):
        ...
        while n < n_trial:
            ...
            r = cs.csUploadData(c_char_p(fi), c_char_p(str(os.stat(fi).st_size)), c_char_p(str(n_all)), buf, byref(c_int(100)))
            if r == 0: 
                dt = mid(buf, 37, 14)    
                ...
                try:
                    ...
                    cr.executemany('insert into tk_upload (tk_id, upload_id) values (?, ?)',
    ...
    def process(self):
        ...
        if n_all:
            ...
            for ii, i in enumerate(r):
                log(u'整理患者 %s %s 案件中： %s/%s （%s%%）' % (self.pid, name, ii + 1, n_all, int((ii + 1) * 100. / n_all))) 
                ...
                r1 = cr.execute('select id, dt from tk where inst_id = ? order by dt', (i[0],)).fetchall()
                for i1 in r1:
        ...
        for k, ct in enumerate(INSTTYPE):          
            ...
            if n_all:                    
                ...
                for ii, i in enumerate(r):
                    ...
                    log(u'整理 %s 案件中： %s/%s （%s%%）' % (ct, ii + 1, n_all, int((ii + 1) * 100. / n_all))) 
                    if self.is_stopped():
                        abort()
                        return 
...
class stats(thread):
    ...
    def process(self):
        ...
        r = cr.execute('''select tk.id, tk.dt 
               ...
               and (exists (select 1 from tk_ord where tk_ord.tk_id = tk.id) or 
                    exists (select 1 from tk_med where tk_med.tk_id = tk.id))
                          order by tk.dt''', (self.ym + '%',)).fetchall()
        ...
        r = cr.execute('''select insttype.code, count(distinct tk.inst_id) 
                   ...
                   and (exists (select 1 from tk_ord where tk_ord.tk_id = tk.id) or 
                        exists (select 1 from tk_med where tk_med.tk_id = tk.id))
                              group by insttype.code''', (self.ym + '%', )).fetchall()
        ...
        self.emit(SIGNAL('done'), 
                (json.dumps({'title': u'%s 每日點數一覽圖（ 案件分佈：%s / 累積總點數：%s ）' % (d1, ' '.join([u'[%s] %s' % i for i in r]), cum), 'l': l[ww:] + l[0:ww],}), ','.join(wd[ww:] + wd[0:ww]))) 
...
class printer(thread):
    ...
    def process(self):
        ...
        for i, c in enumerate(r):
            ...
            for typ in ['dtl', 'rcpt', 'rcptn', 'prscr', 'chk','referral', 'proof']:
...
class model_ord(model):
    ...
    def data(self, ix, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            ...
            if col == ORD_CODE:
                return QVariant(code if code.find('ord***') == -1 else u'（自訂處置）')
...
class model_med(model):
    ...
    def data(self, ix, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            ...
            if col == MED_CODE:
                return QVariant(code if code.find('med***') == -1 else u'（自訂藥品）')
...
class model_med_fav(model):
    ...
    def data(self, ix, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            med_info = dic_all_inv['med'].get(l[1])
            code, name = med_info[:2]
            manufacturer = med_info[-2]
            price = get_price('med', code, dt)
            usage_code = dic_all_inv['usage'].get(l[2])[0]
            freq_code, _, freq_freq = dic_all_inv['freq'].get(l[3])
            ocode_name = dic_all_inv['ocode'].get(l[4])[1]
            ...
            elif col == MED_MANUFACTURER:
                return QVariant(manufacturer[:4])
...
class model_price(model):

    def __init__(self, ls, meta):
        super(model_price, self).__init__(ls, meta)
    ...
    def data(self, ix, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            if col == PRICE_DATE:
                b = l[0] 
                return QVariant('/'.join([str(int(b[:3]) + 1911), b[3:5], b[5:]]))

            elif col == PRICE_PRICE:
                return QVariant(l[1])
...
class delg_price(QStyledItemDelegate):

    def __init__(self, par=None):
        super(delg_price, self).__init__(par)
    ...
    def createEditor(self, par, opt, ix):
        ...
        if col == PRICE_PRICE:
            dsp = QDoubleSpinBox(par)
            dsp.setSingleStep(0.01)
            dsp.setMaximum(10000000)
            return dsp
        
        elif col == PRICE_DATE:
            ded = QDateEdit(par)
            ded.setDisplayFormat('yyyy/MM/dd')
    ...
    def setEditorData(self, ed, ix):
        ...
        if col == PRICE_PRICE:
            ...
            ed.setDate(QDate.fromString(txt, 'yyyy/MM/dd'))
    ...
    def setModelData(self, ed, md, ix):
        ...
        if col == PRICE_PRICE:
            ...
            md.setData(ix, QVariant(dt_tw(ed.date().toPyDate(), date_only=True)))
    ...
    #        code = l['code']
    #        name = txt
    #    r = cr.execute('select * from fav where code = ? and name = ?', (code, name)).fetchone()
    #    if r:
...
class model_med_self(model):

    def __init__(self, ls, meta):
        super(model_med_self, self).__init__(ls, meta)
    ...
    def data(self, ix, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            ...
            elif col == MED_CODE_SELF:
                return QVariant(l['code'])
            
            elif col == MED_NAME_SELF:
                return QVariant(l['name'])
...
class delg_med_self(QStyledItemDelegate):

    def __init__(self, par=None):
        super(delg_med_self, self).__init__(par)
    ...
    def setModelData(self, ed, md, ix):
        ...
        l = md.ls[row]
        if col == MED_NAME_SELF:
            code = l['code']
            name = txt
        r = cr.execute('select * from med where code = ? and name = ?', (code, name)).fetchone()
        if r:
            QMessageBox.warning(self.parent(), u'輸入藥品錯誤', u'此藥品「代碼」/「名稱」重複存在。\n請重新修改後再嘗試存檔！')
        else:
            md.setData(ix, QVariant(ed.text()))

class model_ord_self(model):

    def __init__(self, ls, meta):
        super(model_ord_self, self).__init__(ls, meta)
    ...
    def data(self, ix, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            ...
            elif col == ORD_CODE_SELF:
                return QVariant(l['code'])
            
            elif col == ORD_NAME_SELF:
                return QVariant(l['name'])
...
class delg_ord_self(QStyledItemDelegate):

    def __init__(self, par=None):
        super(delg_ord_self, self).__init__(par)
    ...
    def setModelData(self, ed, md, ix):
        ...
        l = md.ls[row]
        if col == ORD_NAME_SELF:
            code = l['code']
            name = txt
        r = cr.execute('select * from ord where code = ? and name = ?', (code, name)).fetchone()
        if r:
            QMessageBox.warning(self.parent(), u'輸入處置錯誤', u'此處置「代碼」/「名稱」重複存在。\n請重新修改後再嘗試存檔！')
        else:
            md.setData(ix, QVariant(ed.text()))
    ...
    def paintEvent(self, e):
        ...
        for i in range(n):
            ...
            p.drawEllipse(self.width() / 2 + 30 * math.cos(2 * math.pi * i / (n * 1.)) - 10,
                          self.height() / 2 + 30 * math.sin(2 * math.pi * i / (n * 1.)) - 10, 14, 14)
        p.end()
...
class wdg_self(QWidget, Ui_wdg_self):

    def __init__(self, par=None):
        QWidget.__init__(self, par)
        self.setupUi(self)

class tbv_context_menu(QTableView):

    def __init__(self, par=None):
        QTableView.__init__(self, par)
    ...
    def contextMenuEvent(self, e):
        ...
        for ll in l:
            if ll is None:
                ...
                s, f = ll
                m.addAction(s, partial(self.handler_act, f))
        
        self.p = e.pos() 
        m.exec_(e.globalPos())
    
    def handler_act(self, i):
        ix = self.indexAt(self.p)
        if ix is None:
            return
        if i == 'copy':
            m = ix.model()
            app.clipboard().setText(m.data(m.index(ix.row(), ix.column()), Qt.DisplayRole).toString())

class wdg_probe(QWidget):
    ...
    def __init__(self, par=None):
        ...
        font.setWeight(75)
        self.setFont(font)
        self.hlo = QHBoxLayout(self)
        self.spl = QSplitter(Qt.Vertical, self)
        self.led_find = QLineEdit(self.spl)
        font = QFont()
        font.setFamily('Noto Sans Mono CJK TC')
        font.setBold(False)
        font.setWeight(50)
        self.led_find.setFont(font)
        self.tbv_diag = tbv_context_menu(self.spl)
        self.tbv_ord = tbv_context_menu(self.spl)
        self.tbv_mat = tbv_context_menu(self.spl)
        self.tbv_med = tbv_context_menu(self.spl)

        self.hlo.addWidget(self.spl)

        for t in ['tbv_diag', 'tbv_ord', 'tbv_mat', 'tbv_med']:
            w = getattr(self, t).verticalHeader()
            w.setResizeMode(QHeaderView.Fixed)
            w.setDefaultSectionSize(30)

class wdg_fav(QWidget):

    def __init__(self, par=None):
        QWidget.__init__(self, par)
        self.vlo = QVBoxLayout(self)
        self.led_search = QLineEdit(self)
        self.tbv = tbv_context_menu(self)
        self.vlo.addWidget(self.led_search)
        self.vlo.addWidget(self.tbv)

class wdg_search(QWidget):

    def __init__(self, par=None):
        super(wdg_search, self).__init__(par)
        ...
        self.connect(self.led_search, SIGNAL('returnPressed()'), partial(self.handler_act, 'search'))
    ...
    def handler_act(self, i):
        if i == 'focus':
            ...
            s = unicode(self.led_search.text())
            ix = ix_whoosh 
            with ix.searcher() as searcher:
                query = QueryParser('name', ix.schema).parse(s)
                results = searcher.search(query, limit=100)
                if len(results) == 0:
                    self.browser.set_htm('<html><body><h1>No Results using keyword [ %s ] !!</h1></body></html>' % s)
                    return
                html = '<html><body>'
                for i in results[1:100]:
                    html += """<p><a href='""" + i['code'] + """' onClick="mycis.send_msg('""" + i['iid'] + """')">""" + i['code'] + """</a><br /><span class="name">""" + i['name'] + """</span><br />""" + '' + """</p>"""    
                #i.highlights('name'), i.score
                html += '</body></html>'
                b = self.browser
                b.set_htm(html)
                self.js = mycis_js(self)
                b.page().mainFrame().addToJavaScriptWindowObject('mycis', self.js)
...
class mycis_js(QObject):
    ...
    def send_msg(self, msg):
        self.par.emit(SIGNAL('got_id'), msg)
        #self.par.close()
...
class browser(QWebView):
    ...
    def __init__(self, par=None):
        ...
        self.set_htm('<html><body></body></html>')

    def set_htm(self, h='', ref='/home/cytu/sample.html'):
        self.setHtml(h, QUrl.fromLocalFile(ref if ref else __file__))
...
class win_ic(win, Ui_win_ic):
    ...
    def __init__(self, par=None):
        ...
        for i in ['get_basic', 'get_diag', 'get_ord', 'get_allergy',
                  ...
                  'new_tab']:
            self.connect(getattr(self, 'act_%s' % i), SIGNAL('triggered()'), partial(self.handler_act, i))
    ...
    def handler_dll(self, d):
        ...
        if f == 'get_basic':
            ...
            if aim == 'set_allergy':
                ...
                if r == 0:
                    tel, ok = QInputDialog.getText(self, u'緊急聯絡電話輸入', u'請輸入緊急聯絡電話（最大長度：十五個字元）')
                    
                    if ok and not tel.isEmpty():
                        self.dll.go(f='set_tel', kw={'pid': d['pid'], 'birthday': dd['birthday'],'tel': unicode(tel)})
                else:
                    show_error(self, r)
                ...
                model_dsp(ls=t3, 
                        meta={'caption': u'重要醫令',
                              'header': (u'實施日期', 
                                         ...
                                         u'使用部位', 
                                         u'總量')})
                model_dsp(ls=t4, meta={'caption': u'過敏藥物', 'header': (u'過敏藥物成分',)})
            ...
            if r == 0:
                w = self.tw.currentWidget()
                model_dsp(ls=t, meta={'caption': u'緊急聯絡電話', 'header': (u'緊急聯絡電話',)})
            else:
                show_error(self, r)
            ...
            if r == 0:
                w = self.tw.currentWidget()
                model_dsp(ls=t, meta={'caption': u'器官捐贈註記', 'header': (u'器官捐贈註記',)})           
            else:
                show_error(self, r)
...
class win_main(win, Ui_win_main):
    ...
    def __init__(self):
        ...
        for i in ['tk', 'quit', 'inst', 'stat', 'renew', 'app', 'backup', 'about', 'patient', 'self', 'fav']:
            self.connect(getattr(self, 'act_%s' % i), SIGNAL('triggered()'), 
                    partial(self.handler_act, i))
        ...
        self.ded.setDate(QDate(2016, 3, 1))#QDate.currentDate())
        ...
        self.connect(r, SIGNAL('stopped'), partial(self.status, u'終止掛號紀錄整理！'))
        
        self.uploader = u = uploader(self)
        self.connect(u, SIGNAL('status'), self.status)
        self.connect(u, SIGNAL('stopped'), partial(self.status, u'終止上傳作業！'))
        ...
        for j in ['probe',]:
            ...
            for i in getattr(self, 'item_%s' % j):                
                if i is None:
                    ...
                    a.setCheckable(ck)
                    if n in ['prscr', 'rcpt', 'rcptn', 'dtl', 'chk', 'proof', 'referral']:
                        self.connect(a, SIGNAL('triggered()'), partial(self.printf, n))
                    else:
                        self.connect(a, SIGNAL('triggered()'), getattr(self, n))
                    mnu.addAction(a)
        
        self.connect(self.ded, SIGNAL('dateChanged(QDate)'), self.refresh)

        tbv = self.tbv
        self.connect(tbv.verticalHeader(), SIGNAL('sectionClicked(int)'), self.ent_tk)
        tbv.setFocus()
            ...
            if dlg.exec_():
                ...
                if slc == 3:
                    ...
                    self.dll.go(f='get_basic',
                                var={'slc': slc, 'amend_code': amend_code, 'tktype_code': tktype_code,
    ...
    def update_tbv(self, ls):
        tbv = self.tbv
        tbv.setModel(
            model_tk(ls=ls, meta={'header': (u'', u'患者姓名', u'身份證號', u'出生日期', u'掛號時間', u'就醫序號', u'案件號碼')}))      
        ...
        if f == 'get_basic':
            ...
            if r == 0:
                ...
                if var.get('aim', '') == 'fast_amend':
                    ...
                    d2 = tk_save(name=dd['name'], pid=dd['pid'], birthday=dd['birthday'], dt=dt_tw(dt_before(dt_now())))
                    ...
                    if amend_code == '2' or tktype_code == 'AA':
                        ...
                        dlg1 = dlg_ent_inst(par=self, pid=dd['pid'], name=dd['name'],  
                                            birthday=dd['birthday'], amend_code=amend_code)
                        if dlg1.exec_():
                            inst_id = dlg1.inst_id
                            if not inst_id:
                                show_error(self, title=u'無法實施%s掛號' % tp, msg=u'無符合%s條件之起始案件可供選擇！' % tp)

                                dlg1.close()
                                self.overlay.hide()
                                return 
                        else:
                            #show_error(self, title=u'取消%s掛號' % tp, msg=u'使用者取消%s案件選擇動作！' % tp)
                            #dlg1.close()
                            self.overlay.hide()
                            return  
                ...
                try:
                    ...
                    if newbornmark:
                        ans = qna(self, u'使用新生兒註記掛號', u'是否使用新生兒註記掛號？', QMessageBox.Yes|QMessageBox.No|QMessageBox.Cancel)
                        if ans == QMessageBox.No:
                            newborn, newbornmark, newbornprobe = '', '', ''
                            
                        elif ans == QMessageBox.Cancel:
                            show_error(self, title=u'取消 IC卡掛號', msg=u'使用者取消 IC卡掛號！')
                ...
                try:
                    cr.execute('''update patient set name = ?, birthday = ? where pid = ?''', (dd['name'], dd['birthday'], dd['pid'])) 
                    cn.commit()
                except:
                    pass

                self.dll.go(f='get_seq', var=var, kw={'amend_code': amend_code, 'tktype_code': tktype_code, 'd': dd})
            ...
            if r == 0:
                ...
                self.dll.go(f=('init_sam' if r in (4029, 4042) else 'update_hc'), 
                            f_next='get_seq', var=var,
                            kw_next={'amend_code': var['amend_code'], 'tktype_code': var['tktype_code'], 'd': dd})
            else:
                var['r'] = r
                self.tk_stop(var)
            ...
            if r == 0:
                ...
                if f == 'init_sam' and self.n_trial < n_sam_trial:
                    ...
                    #                                   (self.n_trial + 1,))
                    time.sleep(3) 
                    self.dll.go(f='init_sam', f_next=d.get('f_next', ''), kw_next=d.get('kw_next', {}), var=var)
                    self.n_trial += 1
            ...
            if r == 0:
                if self.remove_tk(*arg):
                    info(self, u'IC卡退掛成功', u'%s    %s\n%s\nIC卡序『 %s 』\n掛號資料退掛成功！' % tp)
                else:
                    # XXX
                    logger.error('tk deletion error -- %s %s %s [%s]'% tp) 
            else:
                show_error(self, r, u'IC卡退掛失敗', u'%s    %s\n%s\nIC卡序『 %s 』\n無法退掛，請檢查！' % tp)
    ...
    def tk_nic(self, tktype_code='01', serial='B000', insttype_code='09',inst_id=0):
        ...
        if dlg.exec_():
            ...
            if tktype_code == 'AA': 
                serial = ''
                dlg1 = dlg_ent_inst(par=self, pid=pid, name=name, birthday=birthday, insttype_code=insttype_code)
                if dlg1.exec_():
                    inst_id = dlg1.inst_id
                    if not inst_id:
                        show_error(dlg1, title=u'無法實施同一療程掛號', msg=u'無原始同一療程案件可供選擇！')
                        return
                else:
                     #show_error(self, title=u'取消同一療程掛號', msg=u'使用者取消同一療程案件選擇動作！')
                     dlg1.close()
                     return
            
            dt = dlg.dted_tk.dateTime()
            tk_save(name=name, pid=pid, birthday=birthday, tktype_code=tktype_code, 
                    dt=dt_tw(dt.toPyDateTime()), serial=serial, insttype_code=insttype_code, inst_id=inst_id)
            # XXX 
    ...
    def remove_tk(self, row, tk_id, inst_id):
        try:
            ...
            for t in ['tk_diag', 'tk_ord', 'tk_med', 'tk_mat', 'tk_upload',]:
                cr.execute('delete from %s where tk_id = ?' % t, (tk_id,))
            cr.execute('delete from tk where id = ?', (tk_id,)) 
            
            # delete inst_id if #tk = 0  
            r = cr.execute('select 1 from tk where inst_id = ?', (inst_id,)).fetchone() 
            if r is None:
    ...
    def keyPressEvent(self, e):
        ...
        if key == Qt.Key_Delete:
            ...
            if cr.execute('select 1 from inst_app where inst_id = ?', (inst_id,)).fetchone() is not None:
            ...
            if serial.isdigit():
                if not is_dll_ok or not is_com_ok:
                    ...
                    self.dll.go(f='unget_seq', kw={'dt': dt}, var={'tp': tp, 'arg': (row, tk_id, inst_id)})
                    self.overlay.show()

            else:
                if self.remove_tk(row, tk_id, inst_id):
                    info(self, u'退掛成功', u'%s    %s\n%s\n序號『 %s 』\n之掛號資料已從資料庫刪除！' % tp)
    ...
    def closeEvent(self, e):
        ...
        try:
            tbv = self.tbv 
            for i in range(tbv.model().columnCount()):
                sts.setValue('%s/tbv/%s/width' % (n, i), QVariant(tbv.columnWidth(i)))
        except:
            pass
...
class win_self(win):
    ...
    def __init__(self, par=None):
        win.__init__(self, par)
        self.setStyleSheet(style_sheet_master)
        self.setWindowTitle(u'自訂藥品處置管理')
        self.setWindowIcon(QIcon(':/res/img/pill.png'))

        t = QTabWidget(self)
        t.setTabPosition(QTabWidget.South)
        t.setTabsClosable(False)
        t.setDocumentMode(False)
        t.addTab(wdg_self(), QIcon(':/res/img/pill.png'), u'藥品')  
        t.addTab(wdg_self(), QIcon(':/res/img/syringe.png'), u'處置')  
        self.setCentralWidget(t)
        
        self.stb = QStatusBar(self)
        ...
        d_selfs = import_selfs(cr) 
        self.d_selfs_ini = deepcopy(d_selfs)
        self.d_selfs = deepcopy(d_selfs)
        QTimer.singleShot(0, self.init)
    
    def update_tbv_self(self, i):
        tbv = self.centralWidget().widget(i).tbv
        if i == 0:
            model = model_med_self(ls=self.d_selfs[i], meta={'header': (u'id', u'代碼', u'自訂藥品名稱')})
            delg = delg_med_self(self)
        else:
            model = model_ord_self(ls=self.d_selfs[i], meta={'header': (u'id', u'代碼', u'自訂處置名稱')})
            delg = delg_ord_self(self)
        tbv.setModel(model)     
        tbv.setItemDelegate(delg)
        tbv.setColumnHidden(0, True)
        tbv.setColumnHidden(1, True)
        self.connect(tbv.selectionModel(), SIGNAL('currentChanged(QModelIndex, QModelIndex)'), self.price_item)

    def update_tbv_price(self, i, j):
        tbv = self.centralWidget().widget(i).tbv_price
        model = model_price(ls=self.d_selfs[i][j]['price'], meta={'header': (u'生效日期', u'價格')})
        delg = delg_price(self)
        tbv.setModel(model)     
        tbv.setItemDelegate(delg)

    def resize_tbvs(self): 
        for i in range(2):
            for t in ['tbv', 'tbv_price']:
                tbv = getattr(self.centralWidget().widget(i), t)
                for j in range(tbv.model().columnCount()):
                    n, ok = sts.value('%s/%s/%s/%s/width' % (self.class_name(), t, i, j)).toInt()
                    if ok:
                        tbv.setColumnWidth(j, n)

    def price_item(self, ix, ix0):
        sender = self.sender()
        tab = -1
        for i in range(2):
            if sender is self.centralWidget().widget(i).tbv.selectionModel():
                tab = i
                break
        if tab == -1:
            return
        self.update_tbv_price(tab, ix.row())

    def init(self):
        w = self.centralWidget()
        for i in range(2):
            w.widget(i).spl.restoreState(sts.value('%s/%s/%s' % (self.class_name(), 'spl', i)).toByteArray())
            if self.d_selfs[i]: 
                self.update_tbv_self(i)
                w.widget(i).tbv.selectRow(0)
        self.resize_tbvs()
        w.setCurrentIndex(0) 

    def handler_act(self, i):
        if i == 'add_self':
            tab = self.centralWidget().currentIndex()
            if tab == -1:
                return
            dlg = dlg_self(self) 
            dlg.setWindowTitle(u'自訂%s輸入' % (u'處置' if tab else u'藥品'))
            dlg.setWindowIcon(QIcon(':/res/img/%s.png' % ('syringe' if tab else 'pill')))
            if dlg.exec_():
                self.d_selfs[tab].append({
                    'name': unicode(dlg.led_name.text()), 
                    'code': '', 
                    'id': 0,
                    'price': [[dt_tw(dlg.ded_effect.date().toPyDate(), date_only=True), dlg.dsp_price.value()]] 
                })
                self.update_tbv_self(tab)
                row = len(self.d_selfs[tab]) - 1
                tbv = self.centralWidget().widget(tab).tbv
                tbv.selectRow(row)
                tbv.showRow(row)
            ...
            dlg = dlg_price(self) 
            if dlg.exec_():
                j = w.widget(tab).tbv.selectedIndexes()[0].row()
                self.d_selfs[tab][j]['price'].append([dt_tw(dlg.ded_effect.date().toPyDate(), date_only=True), dlg.dsp_price.value()]) 
                self.update_tbv_price(tab, j)
            ...
            for ii in range(2):
                d_selfs_i = self.d_selfs_ini[ii]
                d_selfs_f = self.d_selfs[ii]
                d_selfs = diff_d_selfs(d_selfs_i, d_selfs_f)
                typ = 'ord' if ii else 'med'
                for d_self in d_selfs['l_insert']:
                    cr.execute('insert into %s (code, name) values (?, ?)' % typ, ('*', d_self['name']))
                    iid = cr.execute('select last_insert_rowid()').fetchone()[0]
                    cr.execute('update %s set code = ? where id = ?' % typ, ('%s***%s' % (typ, iid), iid))
                    
                    for (date, price) in d_self['price']:
                        r = cr.execute('select id from effect where effective_date = ?', (date,)).fetchone()
                        if r:
                            effect_id = r[0]
                        else:
                            cr.execute('insert into effect (effective_date) values (?)', (date,))
                            effect_id = cr.execute('select last_insert_rowid()').fetchone()[0]
                        cr.execute('insert into %s_effect (%s_id, effect_id, price) values (?, ?, ?)' % tuple([typ]*2), (iid, effect_id, price))

                id_del = [(d_self['id'],) for d_self in d_selfs['l_delete']]
                #if id_del:
                #    cr.executemany('''delete from %s_effect 
                #                        where %s_id = ?''' % tuple([typ]*2), 
                #                       id_del)
                #    # XXX tk_***, fav_***
                #    for _ in ['tk', 'fav']:
                #        cr.executemany('''delete from %s_%s 
                #                          where %s_id = ?''' % (_, typ, typ), 
                #                          id_del)
                #    cr.executemany('delete from %s where id = ?' % typ, id_del)
                    
                for nn, d_self in enumerate(d_selfs['l_update_i']):
                    d_self1 = d_selfs['l_update_f'][nn] 
                    if d_self['name'] != d_self1['name']:
                        cr.execute('update %s set name = ? where id = ?' % typ, (d_self1['name'], d_self1['id']))

                    d = diff(d_self['price'], d_self1['price'], n=1)
                    iid = d_self['id']

                    for (date, price) in d['l_insert']:
                        r = cr.execute('select id from effect where effective_date = ?', (date,)).fetchone()
                        if r:
                            effect_id = r[0]
                        else:
                    ...
                    for (date, price) in d['l_update']:
                        effect_id = cr.execute('select id from effect where effective_date = ?',(date,)).fetchone()[0]
                        cr.execute('update %s_effect set price = ? where %s_id = ? and effect_id = ?' % tuple([typ]*2), (price, iid, effect_id))

                    for (date, price) in d['l_delete']:
                        effect_id = cr.execute('select id from effect where effective_date = ?',(date,)).fetchone()[0]
                        cr.execute('delete from %s_effect where %s_id = ? and effect_id = ?' % tuple([typ]*2), (iid, effect_id))
                        # XXX check if effect_id is dangling
                        b_empty = True 
                        for t in ['ord', 'med', 'mat']:
                            r = cr.execute('''select * from %s_effect 
                                where effect_id = ? limit 1''' % t).fetchone()
                            if r:
                                b_empty = False
                                break 
                        if b_empty:
                            cr.execute('delete from effect where id = ?', (effect_id,))

            cn.commit()
            self.d_selfs_ini = deepcopy(self.d_selfs)
            update_dic_price(cr)        
            update_dic_all(cr)
    ...
    def keyPressEvent(self, e):
        if e.key() == Qt.Key_Delete:
            w0 = self.centralWidget()
            w = w0.currentWidget()
            tab = w0.currentIndex()
            tbv = None
            for s in ['tbv', 'tbv_price']:
                t = getattr(w, s)
                if t.hasFocus():
                    tbv = t
                    break
    ...
    def save_widgets_pos(self):
        ...
        for i in range(2):
            sts.setValue('%s/%s/%s' % (class_name, 'spl', i), 
                QVariant(getattr(w.widget(i), 'spl').saveState()))
            
            for t in ['tbv', 'tbv_price']:
                tbv = getattr(w.widget(i), t)
                for j in range(tbv.model().columnCount()):
                    sts.setValue('%s/%s/%s/%s/width' % (class_name, t, i, j), 
...
class win_fav(win, Ui_win_fav):
    ...
    def __init__(self, par=None, staff_id=1):
        ...
        #self.setWindowIcon(QIcon(':/res/img/user_%s.png' % gender(tk['pid'])))
        #self.setWindowTitle(u'  '.join([tk['name'], u'%s 歲' % (age(tk['birthday']),), 
        #                                #dt_s(tk['birthday'], True), 
        #                                tk['addr'], tk['tel']]))
        
        # setup dwgs
        self.dwg = dwg = QDockWidget(u'代碼總覽', self)
        dwg.setObjectName('dwg') 
        dwg.setFeatures(QDockWidget.DockWidgetFloatable|QDockWidget.DockWidgetMovable)

        t = QTabWidget(self)
        t.setTabPosition(QTabWidget.South)
        t.setTabsClosable(False)
        t.setDocumentMode(False)
        t.addTab(wdg_fav(), QIcon(':/res/img/favorites.png'), u'代碼名稱')  
        t.addTab(wdg_search(), QIcon(':/res/img/edit-find.png'), u'資料庫搜尋')  
        dwg.setWidget(t)
        self.addDockWidget(Qt.LeftDockWidgetArea, dwg)
        ...
        tbv = self.dwg.widget().widget(0).tbv
        tbv.setShowGrid(False)
        self.b_save = False 
        
        self.connect(self.dwg.widget().widget(0).led_search, SIGNAL('textChanged(QString)'), self.locate_fav)
        self.connect(self.dwg.widget().widget(1), SIGNAL('got_id'), self.enter_item)
        ...
        self.connect(self.wdg.led_find, SIGNAL('textChanged(QString)'), self.test_empty_favs)
    
    def locate_fav(self, s):
        w = self.dwg.widget().widget(0)
        led = w.led_search
        s = unicode(s)
        n = len(s)
        if not n:
            return
        # XXX linear search!
        for ii, fav in enumerate(self.favs):
            if fav['code'][:n] == s:
                w.tbv.scrollTo(w.tbv.model().index(ii, 1), QAbstractItemView.PositionAtTop)
                break
    ...
    def tbv_item(self, ix, ix0):
        self.update_tbv(self.favs[ix.row()])
    
    def update_tbv_favs(self):
        tbv = self.dwg.widget().widget(0).tbv
        tbv.setModel(model_fav(ls=self.favs, meta={'header': (u'', u'代碼', u'名稱')}))     
        tbv.setItemDelegate(delg_fav(self))
        tbv.setColumnHidden(0, True)
        self.connect(tbv.selectionModel(), SIGNAL('currentChanged(QModelIndex, QModelIndex)'), self.tbv_item)
    ...
    def init(self):
        ...
        if favs:
            ...
            self.dwg.widget().widget(0).tbv.selectRow(0)

        for i in ['spl',]:
            getattr(w, i).restoreState(sts.value('%s/%s' % (self.class_name(), i)).toByteArray())
        self.resize_tbv(['tbv', 'tbv_diag', 'tbv_ord', 'tbv_med', 'tbv_mat'])
        self.focus()
    ...
    def handler_act(self, i):
        if i == 'add_fav':
            ...
            if dlg.exec_():
                ...
                tbv.selectRow(row)
                tbv.showRow(row)
                #QTimer.singleShot(0,self.dwg.widget().widget(0).tbv.scrollToBottom)
            ...
            start_time = time.time()
            print(u'beginning transaction ...')
            self.stb.showMessage(u'儲存藥品處置及重建索引檔，請稍候 ...')
            ...
            for fav in favs['l_insert']:
                ...
                cr.execute('''insert into fav (code, name, staff_id) values (?, ?, ?)''', 
                            (fav['code'], fav['name'], self.staff_id))
            ...
            if id_del:
                for t in ['diag', 'ord', 'med', 'mat']:
                    table = 'fav_%s' % t
                    cl = columns(table)
                    cr.executemany('delete from %s where fav_id = ?' % (table,), id_del)
            ...
            led = self.dwg.widget().currentWidget().led_search
            led.setFocus()
            led.selectAll()
    ...
    def update_tbv(self, fav):
        ...
        w.tbv_diag.setModel(model_diag(ls=fav['diag'], meta={'header': (u'ICD10-CM', u'疾病名稱')}))
        w.tbv_diag.show()

        w.tbv_ord.setModel(model_ord_fav(ls=fav['ord'], dt=now(),
                            meta={'header': (u'健保代碼', u'處置名稱', u'成數', u'總數', u'申報模式')}))
        w.tbv_ord.setItemDelegate(delg_ord_fav(self))
        w.tbv_ord.show()

        w.tbv_med.setModel(model_med_fav(ls=fav['med'], dt=now(), 
            meta={'header': (u'健保代碼', u'藥品名稱', u'製造商', u'劑量', u'頻率', u'天數', u'用法', u'申報模式')})) 
        w.tbv_med.setItemDelegate(delg_med_fav(self))
        w.tbv_med.show()
        
        w.tbv_mat.setModel(
            model_mat_fav(ls=fav['mat'], dt=now(),
                          meta={'header': (u'健保代碼', u'特材名稱', u'成數', u'總數', u'申報模式')}))
        w.tbv_mat.setItemDelegate(delg_mat_fav(self))
        w.tbv_mat.show()
        
        for s in ['diag', 'ord', 'med', 'mat']:
            t = getattr(w, 'tbv_%s' % s)
            self.connect(t.model(), SIGNAL('dataChanged(QModelIndex, QModelIndex)'), self.edited)
    ...
    def save_widgets_pos(self):
        ...
        for i in ['tbv', 'tbv_diag', 'tbv_ord', 'tbv_med', 'tbv_mat']:
            if i == 'tbv':
                tbv = self.dwg.widget().widget(0).tbv
            else:
                tbv = getattr(w, i)
            for j in range(tbv.model().columnCount()):
                sts.setValue('%s/%s/%s/width' % (class_name, i, j), QVariant(tbv.columnWidth(j)))
    ...
    def keyPressEvent(self, e):
        if e.key() == Qt.Key_Delete:
            w = self.wdg
            tbv = None
            for s in ['tbv', 'tbv_diag', 'tbv_ord', 'tbv_med', 'tbv_mat']:
                if s == 'tbv':
                    t = self.dwg.widget().widget(0).tbv
                else:
                    t = getattr(w, s)
                if t.hasFocus():
                    tbv = t
            ...
            if s == 'tbv':
                ...
                if favs:
                    ...
                    # clear all views
                    self.update_tbv({'name': '', 'code': '', 'fav_id': 0, 'diag': [], 'ord': [], 'med': [], 'mat': [],})
...
class win_probe(win, Ui_win_probe):
    ...
    def __init__(self, par=None, tk_id=0):
        ...
        t.setDocumentMode(False)
        self.setCentralWidget(t)
        t.addTab(wdg_probe(), QIcon(':/res/img/stethoscope.png'), u'看診')
        t.addTab(wdg_probe(), QIcon(':/res/img/microscope.png'), u'檢驗')
        t.addTab(wdg_probe(), QIcon(':/res/img/kwalletmanager.png'), u'批價')
        w = t.widget(0)
        ...
        self.setWindowIcon(QIcon(':/res/img/user_%s.png' % gender(tk['pid'])))
        self.setWindowTitle(u'  '.join([tk['name'], u'%s 歲' % (age(tk['birthday']),), tk['addr'], tk['tel']]))
        ...
        dwg1.setFeatures(QDockWidget.DockWidgetFloatable|QDockWidget.DockWidgetMovable)
        
        self.ted_note = QTextEdit(self)
        self.ted_soap = QTextEdit(self)
        self.ted_past = browser(self)
        ...
        led = self.tw.widget(0).led_find
        led.setCompleter(self.completer)
        self.connect(led, SIGNAL('returnPressed()'), self.hit_test)
        self.connect(self.completer, SIGNAL('activated(QModelIndex)'), self.hit)
    ...
    def apply_tk_load_rules(self, tk):
        ...
        if not tk['diag']:
            for i in cr.execute('''select distinct tk_diag.diag_id from tk_diag 
                                   ...
                                   where tk.inst_id in 
                                   (select inst_id from tk where id = ?)''', (tk_id,)).fetchall():
    ...
    def init(self):
        ...
        note = cr.execute('''select note from patient 
                             where id = (select patient_id from tk where id = ?)''', (tk['tk_id'],)).fetchone()[0]
        ...
        for (dtt, ii, soap) in cr.execute('''select dt, id, soap from tk 
            ...
            for r in cr.execute('''select distinct diag.code, diag.name_zh from diag
                                   join tk_diag on diag.id = tk_diag.diag_id 
                                   where tk_diag.tk_id = ? order by diag.code''', (ii,)).fetchall():
            ...
            for r in cr.execute('''select ord.code, ord.name from tk_ord
                                      join ord on tk_ord.ord_id = ord.id
                                      join tk on tk_ord.tk_id = tk.id
                                      where tk_ord.tk_id = ? order by ord.code''', (ii,)).fetchall():
                l = [unicode(rr).strip() for rr in r]
                l[0], l[1] = dic_custom_code_name.get(l[0], l[:2])
                d['ord'].append(l)
            ...
            for r in cr.execute('''select med.code, med.name, freq.code, 
                             ...
                             join med on tk_med.med_id = med.id 
                             join freq on tk_med.freq_id = freq.id 
                             where tk_med.tk_id = ? order by med.code''', (ii,)).fetchall():
                l = [unicode(rr).strip() for rr in r]
                l[0], l[1] = dic_custom_code_name.get(l[0], l[:2])
                d['med'].append(l)
            ...
            for r in cr.execute('''select mat.code, mat.name from tk_mat
                                      ...
                                      where tk_mat.tk_id = ? order by mat.code''', (ii,)).fetchall():
                l = [unicode(rr).strip() for rr in r]
                l[0], l[1] = dic_custom_code_name.get(l[0], l[:2])
                d['mat'].append(l)
        ...
        self.connect(self.tw.widget(0).led_find, SIGNAL('textChanged(QString)'), self.test_no_match)

    def test_no_match(self, s):
        led = self.tw.widget(0).led_find
        b_empty = False 
        if s:
            ss = led.completer().currentCompletion()
            if not unicode(ss):
                b_empty = True
        led.setStyleSheet('QLineEdit {background-color: %s;}' % ('pink' if b_empty else 'white',));

    def update_favs(self, d):
        cnd = d['cnd']  
        if cnd == 'saved':
            self.fav_refresher.go(staff_id=1)

    def handler_act(self, i):
        if i == 'add_as_fav':
            dlg = dlg_fav(self) 
            if dlg.exec_():
                cr.execute('''insert into fav (code, name, staff_id) values (?, ?, ?)''', (unicode(dlg.led_name.text()), unicode(dlg.led_code.text()), self.staff_id))
                fav_id = cr.execute('select last_insert_rowid()').fetchone()[0]
            ...
            if tk['diag'] and (tk['ord'] or tk['med'] or tk['mat']):
                ...
                show_error(self, title=u'看診無法完成', msg=u'看診資料未輸入完整，請檢查！\n（原因：%s）' % s)
    ...
    def get_history_item(self, i):
        ...
        self.ted_past.set_htm('<html><body>%s</body></html>' % ('\n'.join(l)))

    def update_tbv(self, tk):
        w = self.tw.widget(0)
        w.tbv_diag.setModel(
            model_diag(ls=tk['diag'], meta={'header': (u'ICD10-CM', u'疾病名稱')}))
        w.tbv_diag.show()

        w.tbv_ord.setModel(model_ord(ls=tk['ord'], dt=tk['dt'],
                           meta={'header': (u'代碼', u'處置名稱', u'單價', u'成數', u'總數', u'小計', u'申報模式')}))
        w.tbv_ord.setItemDelegate(delg_ord(self))
        w.tbv_ord.show()

        w.tbv_med.setModel(model_med(ls=tk['med'], dt=tk['dt'],
                           meta={'header': (u'代碼', u'藥品名稱', u'劑量', u'頻率', u'天數', u'總數', 
                                            u'用法', u'單價', u'小計', u'申報模式')}))           
        w.tbv_med.setItemDelegate(delg_med(self))
        w.tbv_med.show()
        
        w.tbv_mat.setModel(
            model_mat(ls=tk['mat'], dt=tk['dt'],
                       meta={'header': (u'代碼', u'特材名稱', u'單價', u'成數', u'總數', u'小計', u'申報模式')}))
    ...
    #    w = self.tw.widget(0)
    #    r = cr.execute('select code, name, id from insttype where code = ?', (insttype_code,)).fetchone()
    ...
    def hit_test(self):
        ix = self.completer.currentIndex()
        if not ix or not ix.isValid():
            return
        m = ix.model()
        self.enter_item(unicode(m.data(m.index(ix.row(), ix.column() + 2), Qt.EditRole).toString()))
        QTimer.singleShot(0, self.focus)
    ...
    def save(self):
        try:      
            ...
            if note != tk_i['note'].strip():
                cr.execute('''update patient set note = ? where id = (select patient_id from tk where id = ?)''', (note, tk_id)) 
...
class win_inst(win_inst_base):
    ...
    def __init__(self, par=None):
        ...
        self.init_w(['lbl_ded', 'ded', '', 'lbl_cbo', 'cbo', '', 'pbn', ''], inst_selector(self))
...
class dlg_ent_inst(dlg, Ui_dlg_ent_inst):
    ...
    def __init__(self, par=None, pid='', name='', birthday='', insttype_code='', inst_id=0, amend_code='1'):
        ...
        self.connect(self.tbv.verticalHeader(), SIGNAL('sectionClicked(int)'), self.ent_inst)
        QTimer.singleShot(0, self.init)
    
    def init(self): 
        self.lbl_gender.setPixmap(QPixmap(':/res/img/user_%s.png' % gender(self.pid)))
        self.lbl_details.setText('\n'.join((self.name, self.pid, dt_s(self.birthday, True),))) 
        ...
        self.tbv.setModel(model_dsp(ls=all_, meta={'caption': u'選擇案件', 'header': (u'', u'掛號時間', u'診斷')}))  
        try:
            n = self.class_name()
            self.restoreGeometry(sts.value('%s/geometry' % n).toByteArray())
...
class dlg_self(dlg, Ui_dlg_self):
    ...
    def __init__(self, par=None):
        ...
        ded = self.ded_effect
        ded.setDisplayFormat('M/d/yyyy')
        ded.setCalendarPopup(True)
        ded.setDate(QDate.currentDate())

class dlg_price(dlg, Ui_dlg_price):
    ...
    def accept(self):
        if not validate_effect(self.ded_effect, self.par):
            return        
        QDialog.accept(self)
...
class dlg_tk_abn(dlg, Ui_dlg_tk_abn):
    ...
    def lookup(self):
        ...
        r = cr.execute('select name, birthday from patient where pid = ?', (pid,)).fetchone()
        if r:    
...
class dlg_print(dlg, Ui_dlg_print):
    ...
    def __init__(self, par=None, ls=None, dtl_only=False):
        ...
        typs = ['chk', 'prscr', 'rcpt', 'rcptn', 'dtl', 'proof', 'referral']
    ...
    def accept(self): 
        ...
        n = self.class_name()
        for i in range(m.columnCount()):
            sts.setValue('%s/tbv/%s/width' % (n, i), QVariant(self.tbv.columnWidth(i)))
        sts.setValue('%s/geometry' % n, QVariant(self.saveGeometry()))
        QDialog.accept(self)
...
show_msg(u'載入健保病名、處置、藥品大檔 ...')
start_time0 = time.time()
model_orig = model_collect(now())
print('load grand model: %f seconds.' % (time.time() - start_time0,))
print('===================================================')
print('Until now, %f seconds have passed.' % (time.time() - start_time1,))
print('===================================================')
